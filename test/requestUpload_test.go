...
func performUpload(t *testing.T, projectRoot, branch, runtimeVersion, sampleUpdatePath, platform string) string {
	...
	r.Body = io.NopCloser(bytes.NewReader(uploadRequestsInputJSON))
	newTestContainer(t).UploadHandler.RequestUploadUrlHandler(w, r)
	if w.Code != 200 {
	...
	for i, uploadRequest := range fileUploadRequests {
		...
		go func(index int, req bucket.FileUploadRequest) {
			...
			newTestContainer(t).UploadHandler.RequestUploadLocalFileHandler(ws[index], uploadReq)
...
func markUpdateAsUploaded(t *testing.T, branch, runtimeVersion, updateId, platform string) *httptest.ResponseRecorder {
	...
	r.Header.Set("Authorization", "Bearer expo_test_token")
	r = mux.SetURLVars(r, map[string]string{"APP_ID": "test-app-id", "BRANCH": branch})
	newTestContainer(t).UploadHandler.MarkUpdateAsUploadedHandler(w, r)
	return w
}
...
func TestRequestUploadUrlWithSampleUpdate(t *testing.T) {
	...
	for i, req := range uploadRequests {
		...
		go func(index int, uploadReq bucket.FileUploadRequest) {
			...
			newTestContainer(t).UploadHandler.RequestUploadLocalFileHandler(ws[index], uploadFileReq)
			if ws[index].Code != 200 {
	...
	newTestContainer(t).UploadHandler.MarkUpdateAsUploadedHandler(wMark, rMark)
...
func TestShouldPreserveCacheOnUploadRequest(t *testing.T) {
	...
	newTestContainer(t).ExpoProtocolHandler.HandleManifest(wManifest, rManifest)
	...
	w := httptest.NewRecorder()
	r := httptest.NewRequest("POST", q, nil)
	r = mux.SetURLVars(r, map[string]string{"APP_ID": "test-app-id", "BRANCH": "branch-1"})
	r.Header.Set("expo-session", "expo_test_session")
	sampleUpdatePath := filepath.Join(projectRoot, "/test/test-updates/test-app-id/branch-1/1/1674170951")
